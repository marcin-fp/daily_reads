...
Bj¨orn Engdahl<sup>1</sup> , Adrian Kosowski<sup>1</sup> , Jan Chorowski<sup>1</sup> , Zuzanna Stamirowska<sup>1</sup> , Przemysław Uzna´nski<sup>1</sup> , Junlin Jiang<sup>1</sup> , Rohan Phadke<sup>1</sup> , Remigiusz Kinas<sup>2</sup> , and Richard Zhong<sup>3</sup> 

> 1Pathway, `https://pathway.com/research` 

> 2Bielik AI `https://bielik.ai/` 
...
Let a task provide demonstrations _D_ = _{_ ( _xt, yt_ ) _}_<sup>_K_</sup> _t_ =1<sup>and query input</sup><sup>_x⋆_. Rather than compressing the demonstrations</sup> into a single task vector, BDH-CQ processes their elements sequentially. At a high level, its recurrent memory evolves as 

where _Dt_ denotes the content of the _t_ -th demonstration and _θ_ remains fixed. Thus, information available to later inputs depends on associations accumulated from earlier inputs and outputs. This recurrent contextual state plays a role analogous to the context-dependent associations constructed by attention, while avoiding a growing explicit key– value cache. The interpretation is generally related to attention, fast-weight memory, and linear-attention views of contextual association (Vaswani et al., 2017; Ba et al., 2016; Katharopoulos et al., 2020; Geva et al., 2021), with linear attention being the conceptually simplest standalone realization of linear correction rules on _S_ , capturing the special case _St_ = _St−_ 1 + _Uθ_ ( _Dt_ ). 

## **3.3 Recurrent latent reasoning** 

After the demonstrations and query have been ingested, BDH-CQ performs iterative computation in a structured latent workspace _Hr_ . This happens after the ingestion of the _K_ demonstrations into memory. We summarize this process through the following equations which capture the encoding of inputs _x_<sup>_⋆_</sup> , latent reasoning, and decoding of the output _y_ ˆ. 

3 

The contextual memory _St_ and reasoning workspace _Hr_ have different conceptual roles. _St_ changes as evidence is encountered and supports in-context learning. _Hr_ carries the ongoing computation used to answer the current query. This organization defines the system-level interface studied in this paper. Dimensions, exact update rules, and implementation details remain proprietary. 

# **4 Training data and objective** 

## **4.1 ARC task formulation** 

An ARC task _Ti_ contains _Ki_ demonstration pairs with _Qi_ test pairs, _Qi ≥_ 1. 
...
**Independent evaluation.** An independent black-box audit conducted by co-authors from Bielik and New York University reproduced the deployed system’s 29.5% pass@2 score on the public ARC-AGI-1 evaluation set. The auditors evaluated the system under a documented protocol without access to model weights. Their report also evaluates BDHCQ on ConceptARC and a hand-crafted evaluation set (Remigiusz Kinas, 2026; Richard Zhong, 2026). 

4 

<!-- Start of picture text -->
ARC-AGI-1 Efficiency Frontier<br>Claude<br>100% > pus Opus55 (Max)(Max)<br>90% ,——e- GPT-5:6 Luna (Max)? :<br>© rom:<br>® an<br>£gg 50% ‘edi Sven na ude Haku 43<br><3 BDH-CQ (High) ‘GPT-5.6Sime'o Lunasaves(Low) . ania PY cide. Open 4<br>© 30% 29.5% @ $0.00070" cost per task aun<br>Medium<br>20%) D Low ww<br>10% | wo Qiven3Row @2358s0.0025 © Pre-BDH.CQPublic Eval SetPareto fronter<br>lew © SemiPrivate Eval Set<br>0%0.0005" $0.01| $0.01 $0.10| sa $10|<br>+ BOH.CQ cost is estimated from measured inference compute, assuming Cost USD  per 3 per  task H200  (USD; GPU-hour, log scale)<br>+2.GPT-5.6 Unless otherwiseLuna costsnated,are asjustedcomparizon-modelto 20% of ARCscoresPrizesand reportedcosts reproducefigures theto reflect ARC PieOpenAl's leaderboard,80% public accessedAP price4 August 2026;reduction effective reported30 costJul 2026;reflect publicscores are APunchanged pricing<br>“TRA and HRM costs are leaderboard-reported compute estimates rather than API prices.<br><!-- End of picture text -->
...
|Set / condition|Unit|_N_|pass@1|pass@2|[95% Wilson]|
|---|---|---|---|---|---|
...
<!-- Start of picture text -->
FilledNotfilled ©—§ Strict-taskTest-pairpass@2pass@2 oe (©© SemanticOpaque iosIbs .<br>‘TopBottom20 oe .<br>ExtendToBoundary . ° oe<br>Cleanup oe .<br>Horizontalvertical . ° .<br>‘TopBottom3D . ° .<br>Extractobjects oe oe<br>Center . . oe<br>Completeshape . ° .<br>Count . ° .<br>AboveBelow ° ° .<br>MoveToBoundary ° ° .<br>InsideOutside . . .<br>samebitferent . ° .<br>copy . . .<br>order ° ° .<br>0% 20% Semantic1D40% accuracy60% 80% 100% 0% 20% Stricttask403% pass@260% 80% 100%<br><!-- End of picture text -->
...
<!-- Start of picture text -->
Propagation to boundary (distance 8): input Oracle output<br>Copy to four anchors: input Oracle output<br>Order eight bars by height: input<br>Oracle output<br>Select inside five nested frames: input Oracle output<br><!-- End of picture text -->
...
<!-- Start of picture text -->
Relocation: input Output<br>Rotation: input Output<br>Relocation + rotation: input Output<br><!-- End of picture text -->
...
On the full public ARC-AGI-1 evaluation, the MIN effort setting cost one third of STANDARD ($0 _._ 00088399 versus $0 _._ 00265246 per task) and scored 111/400 rather than 118/400 pass@2, a difference of _−_ 1 _._ 75 percentage points. The paired split was 105 tasks solved by both settings, 13 standard-only, 6 min-only, and 276 neither (two-sided exact McNemar _p_ = 0 _._ 167). All four reported endpoints favor STANDARD in point estimate, but this comparison is statistically unresolved. 
...
<!-- Start of picture text -->
Pass@2 vs. Cost Across Reasoning Effort Levels<br>HIGH<br>30<br>28<br>MEDIU<br>z<br>5 26<br>g<br>© 24<br>2<br>101<br>20<br>-2 - 0<br>Cost relative to max effort (%)<br><!-- End of picture text -->
...
- ARC Prize Foundation. The Hidden Drivers of HRM’s Performance on ARC-AGI. `https://arcprize.org/blog/ hrm-analysis` , 2025. 

- ARC Prize Foundation. ARC-AGI Leaderboard. `https://arcprize.org/leaderboard` , 2026. Accessed July 2026. 

- J. Ba, G. E. Hinton, V. Mnih, J. Z. Leibo, and C. Ionescu. Using fast weights to attend to the recent past. In _Advances in Neural Information Processing Systems_ , 2016. 

- T. B. Brown et al. Language models are few-shot learners. _Advances in Neural Information Processing Systems_ , 2020. 

- F. Chollet. On the measure of intelligence. _arXiv preprint arXiv:1911.01547_ , 2019. 

- F. Chollet, M. Knoop, G. Kamradt, and B. Landers. ARC Prize 2024: Technical Report. _arXiv preprint arXiv:2412.04604_ , 2024. 

- F. Chollet, M. Knoop, G. Kamradt, B. Landers, and H. Pinkard. ARC-AGI-2: A New Challenge for Frontier AI Reasoning Systems. _arXiv preprint arXiv:2505.11831_ , 2025. 

- DeepSeek-AI, D. Guo, D. Yang, et al. Deepseek-r1: Incentivizing reasoning capability in llms via reinforcement learning. _Nature_ , 645:633–638, 2025. 

- E. Fedorenko and R. Varley. Language and thought are not the same thing: Evidence from neuroimaging and neurological patients. _Annals of the New York Academy of Sciences_ , 1369(1):132–153, 2016. 

- J. Geiping, S. McLeish, N. Jain, et al. Scaling up test-time compute with latent reasoning: A recurrent depth approach. _arXiv preprint arXiv:2502.05171_ , 2025. 

- M. Geva, R. Schuster, J. Berant, and O. Levy. Transformer feed-forward layers are key-value memories. In _Proceedings of the 2021 Conference on Empirical Methods in Natural Language Processing_ , 2021. 

13 

- S. Hao, S. Sukhbaatar, D. Su, et al. Training large language models to reason in a continuous latent space. _arXiv preprint arXiv:2412.06769_ , 2024. 

- M. Hodel. Addressing the abstraction and reasoning corpus via procedural example generation. _arXiv preprint_ , 2024. 

- A. Jolicoeur-Martineau. Less is more: Recursive reasoning with tiny networks. _arXiv preprint arXiv:2510.04871_ , 2025. 

- A. Katharopoulos, A. Vyas, N. Pappas, and F. Fleuret. Transformers are rnns: Fast autoregressive transformers with linear attention. In _International Conference on Machine Learning_ , 2020. 

- A. Kosowski, P. a. Uzna´nski, J. Chorowski, Z. Stamirowska, and M. Bartoszkiewicz. The Dragon Hatchling: The Missing Link between the Transformer and Models of the Brain. _arXiv preprint arXiv:2509.26507_ , 2025. 

- W.-D. Li, K. Hu, C. Larsen, et al. Combining induction and transduction for abstract reasoning. In _International Conference on Learning Representations_ , 2025. 

- Y. Liu. Beyond tokens: A unified framework for latent communication in llm-based multi-agent systems. _arXiv preprint arXiv:2606.05711_ , 2026. 

- M. D. Moffitt. ARC-GEN: A Mimetic Procedural Benchmark Generator for the Abstraction and Reasoning Corpus. _arXiv preprint_ , 2025. 

- A. Moskvichev, V. V. Odouard, and M. Mitchell. The ConceptARC Benchmark: Evaluating Understanding and Generalization in the ARC Domain. _Transactions on Machine Learning Research_ , 2023. 
...
- K. Ramji, T. Naseem, and R. Fernandez Astudillo. Thinking without words: Efficient latent reasoning with abstract chain-of-thought. _arXiv preprint arXiv:2604.22709_ , 2026. 
...
- N. Saunshi, N. Dikkala, Z. Li, S. Kumar, and S. J. Reddi. Reasoning with latent thoughts: On the power of looped transformers. _arXiv preprint arXiv:2502.17416_ , 2025. 

- A. Vaswani, N. Shazeer, N. Parmar, et al. Attention is all you need. In _Advances in Neural Information Processing Systems_ , 2017. 

- G. Wang, J. Li, Y. Sun, et al. Hierarchical reasoning model. _arXiv preprint arXiv:2506.21734_ , 2025. 

- G. Wang, C. Liu, C. Wang, et al. Hrm-text: Efficient pretraining beyond scaling. _arXiv preprint arXiv:2605.20613_ , 2026. 

- J. Wei, X. Wang, D. Schuurmans, et al. Chain-of-thought prompting elicits reasoning in large language models. In _Advances in Neural Information Processing Systems_ , 2022. 

- K. Xu and I. Sato. A formal comparison between chain of thought and latent thought. In _International Conference on Machine Learning_ , 2026. 

- Z. Zhang, X. He, W. Yan, A. Shen, C. Zhao, S. Wang, Y. Shen, and X. E. Wang. Soft thinking: Unlocking the reasoning potential of llms in continuous concept space, 2025. URL `https://arxiv.org/abs/2505.15778` . 

- H. Zhu, S. Hao, Z. Hu, J. Jiao, S. Russell, and Y. Tian. Reasoning by superposition: A theoretical perspective on chain of continuous thought. In _Advances in Neural Information Processing Systems_ , 2025. 

- R.-J. Zhu, Z. Wang, K. Hua, T. Zhang, Z. Li, H. Que, B. Wei, Z. Wen, F. Yin, H. Xing, L. Li, J. Shi, K. Ma, S. Li, T. Kergan, A. Smith, X. Qu, M. Hui, B. Wu, Q. Min, H. Huang, X. Zhou, W. Ye, J. Liu, J. Yang, Y. Shi, C. Lin, E. Zhao, T. Cai, G. Zhang, W. Huang, Y. Bengio, and J. Eshraghian. Scaling latent reasoning via looped language models, 2026. URL `https://arxiv.org/abs/2510.25741` . 
...
|Mechanic|_n_|Solve rate|95% CI|Mean side|
|---|---|---|---|---|
|Flood fill<br>|51<br>|68.6%<br>|[55_,_80]<br>|15.6<br>|
|Denoising|65|56.9%|[45_,_68]<br>|18.8|
|Scaling|82|53.7%|[43_,_64]<br>|15.1|
|Cropping and extraction|74|44.6%<br>|[34_,_56]<br>|19.0|
|Translation|87|35.6%|[26_,_46]<br>|16.6|
|Tiling and repetition|79|34.2%|[25_,_45]<br>|19.5|
|Line drawing<br>|84<br>|29.8%<br>|[21_,_40]<br>|16.6|
|Rotation<br>|75<br>|25.3%<br>|[17_,_36]<br>|14.9|
|Recolor by property|75|25.3%|[17_,_36]<br>|17.0|
|Object sorting and rank<br>|68<br>|19.1%<br>|[12_,_30]<br>|14.9<br>|
|Object counting<br>l|86<br>|18.6%<br>|[12_,_28]<br>|16.7<br>|
|Reflection<br>|72<br>|16.7%<br>|[10_,_27]<br>|17.7<br>|
|Symmetry completion|61|16.4%|[9_,_28]<br>|25.5|
|Occlusion repair<br>|56<br>|16.1%<br>|[9_,_28]<br>|24.3<br>|
|Panel set operation|48|10.4%|[5_,_22]<br>|11.0|
|Gravity and stacking|68|2.9%|[1_,_10]|14.6|
...
|---|---|---|---|
|Conditional rule selection|100.0%|56.7%|_−_43_._3|
|Parameter absent from demonstrations|30.0%|0.0%|_−_30_._0|
|Panel union, two to three panels|65.0%|2.5%|_−_62_._5|
|Support chain, shortest to eight objects|80.0%|27.5%|_−_52_._5|
