...
% \newtcolorbox{summarytablebox}{
%   colback=black!1,
%   colframe=black!30,
%   coltitle=black,
%   title=Competitive landscape in AI for Science,
%   fonttitle=\bfseries,
%   boxrule=0.4pt,
%   arc=2mm,
%   left=2mm,
...
% % Summary page
% \begin{betbox}
% Giants are rewarded for being adequate everywhere. A small team wins by going deep on one niche, such as theoretical physics, and shaping every layer of the system around how the field actually reasons.
% \end{betbox}
...
%   \item \textbf{Scope:} Narrow means sharper interventions and thinner competition. Broad means more data, but also direct competition with Google.

%   \item \textbf{Complexity:} The simplest system that works usually wins. Our advantage should not come from architecture complexity alone.

%   \item \textbf{Physics is hard:} Physics involves approximation, noisy data, implicit assumptions, and questions like ``why'' and ``what if''.

%   \item \textbf{Completeness:} Not every scientific question is an optimization problem. Some require abstractions that generalize back to the real world.

%   \item \textbf{Emergence:} A system tuned to one scientific level may not scale to another without the laws, structures, and data of that higher level.

%   \item \textbf{Modality:} Physics is not only text. Equations, plots, units, symmetries, and causal structure all matter.
% \end{itemize}
% \end{summarypointsbox}
% \end{minipage}
% \hfill
% \begin{minipage}[t]{0.58\linewidth}
% \vspace{0pt}
% \begin{summarytablebox}
% \scriptsize
...
% Google --- Co-Scientist
% &
% General hypothesis generation using multi-agent debate, ranking, review, web search, and databases.
% &
% Deep fit to any single scientific field.
% \\
...
% Google --- ERA
% &
% Expert-level empirical software through tree search over LLM-written code.
% &
% Breadth over depth.
% \\
...
% &
% Token-efficient theorem proving with a minimal Lean~4 feedback loop and lightweight search.
% &
% Specialized training still wins.
% \\
...
% &
% General-purpose frontier models plus agentic loops.
% &
% Excellence in one particular niche.
% \\
...
% \footnotesize
% \emph{Takeaway:} frontier labs optimize for broad coverage. Our opening is depth, domain fit, and structure-aware reasoning in physics.
% \end{summarytablebox}
% \end{minipage}
